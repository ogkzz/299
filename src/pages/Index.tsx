...
import { Shield, Scan, Zap, Lock, ArrowRight, Wifi, Radio, Cpu, Activity, Eye, Layers } from "lucide-react";
...
export default function Index() {
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    try {
      ...
      if (autoBlacklisted) {
        await addToBlacklist(report, events, 'iPhone');
  ...
  const features = [
    ...
    { icon: Eye, title: t('feat.ipintel'), desc: t('feat.ipintel_desc') },
    { icon: Radio, title: t('feat.sideload'), desc: t('feat.sideload_desc') },
    { icon: Activity, title: t('feat.sysdiag'), desc: t('feat.sysdiag_desc') },
  ];
      ...
      <main className="container mx-auto px-4 sm:px-6 pt-24 pb-20">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center max-w-2xl mx-auto mb-10">
          <motion.div
            ...
            className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-primary/5 border border-primary/10 mb-5"
          >
            <Shield className="w-3.5 h-3.5 text-primary animate-dot-pulse" />
            <span className="text-[11px] font-mono text-primary uppercase tracking-widest">{t('hero.badge')}</span>
          </motion.div>
          
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-bold tracking-tight mb-3">
            <span className="gradient-text">Magisk</span>
          </h1>
        ...
        <div className="max-w-2xl mx-auto space-y-3 mb-12">
          ...
          >
            <div className="flex items-center gap-2 mb-2">
              ...
              <label className="text-[11px] text-muted-foreground uppercase tracking-wider font-mono">{t('upload.ip_label')}</label>
            </div>
            <input
              type="text"
              ...
              className="w-full bg-secondary/80 rounded-xl px-4 py-2.5 text-sm text-foreground font-mono placeholder:text-muted-foreground outline-none focus:ring-2 focus:ring-primary/20 transition-shadow"
              maxLength={15}
            />
            <p className="text-[11px] text-muted-foreground mt-2">{t('upload.ip_hint')}</p>
          </motion.div>

          {/* Sysdiagnose guide */}
          <motion.details initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
            className="glass-panel rounded-xl overflow-hidden group"
          >
            <summary className="p-4 cursor-pointer flex items-center gap-2 text-xs text-muted-foreground hover:text-foreground transition-colors">
              <Cpu className="w-3.5 h-3.5" />
              <span className="font-medium">{t('upload.sysdiagnose_guide')}</span>
            </summary>
            <div className="px-4 pb-4 space-y-2">
              {['step1', 'step2', 'step3', 'step4'].map(step => (
                <p key={step} className="text-[11px] text-muted-foreground font-mono">{t(`upload.sysdiagnose_${step}`)}</p>
              ))}
            </div>
          </motion.details>
          ...
          {isAnalyzing && terminalLogs.length > 0 && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }}>
              <TerminalLog logs={terminalLogs} title="magisk-scanner" maxHeight="220px" />
            </motion.div>
          )}
          ...
          {isAnalyzing && (
            ...
            >
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-[11px] text-primary font-mono font-bold">{progress}%</span>
              </div>
              ...
              {networkEvents.length > 0 && (
                <div className="mt-3 space-y-1">
                  {networkEvents.map((ev, i) => (
                    <p key={i} className="text-[11px] font-mono text-destructive bg-destructive/10 px-3 py-1 rounded-lg">⚠ {ev.details}</p>
                  ))}
                </div>
              )}
            </motion.div>
          ...
          <motion.button
            ...
            className="w-full bg-primary text-primary-foreground rounded-xl py-3.5 px-6 font-semibold text-sm flex items-center justify-center gap-2.5 hover:opacity-90 transition-all disabled:opacity-30 disabled:cursor-not-allowed active:scale-[0.98] touch-manipulation shadow-lg shadow-primary/20"
          >
            {isAnalyzing ? (
              <>
        ...
        {/* Features */}
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5 max-w-2xl mx-auto mb-12">
          {features.map((f, i) => (
            <motion.div
              key={f.title}
              initial={{ opacity: 0, y: 15 }}
              ...
              className="glass-panel glass-hover rounded-xl p-4 cursor-default"
            >
        ...
        <div className="max-w-2xl mx-auto">
          <p className="text-center text-[11px] text-muted-foreground uppercase tracking-widest font-mono mb-4">
            {t('results.preview')}
          </p>
          <DashboardPreview />
        </div>
